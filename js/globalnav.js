...
jQuery(document).ready(function(){
    ...
    if (jQuery('#calendar-nav').length) {
        ...
        jQuery('<input type="text" />').dateinput({
            firstDay: 1,
            lang: jQuery('html').attr('lang'),
            onHide: function() { return false; },
            change: function() {
          		window.location = 'http://memento.epfl.ch/cgi-bin/memento2010/select?memento=EPFL&amp;period_ATDATE=1&amp;date=' + this.getValue('yyyy-mm-dd');
        	}                    
        }).data("dateinput").show();

        jQuery('#calroot').detach().appendTo(jQuery('#calendar-nav')).css({
            position:'relative'
        });        
    }
