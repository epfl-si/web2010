 ...
 * 
 * [jquery, tabs, tooltip, overlay, toolbox.expose, jquery.ui.core, jquery.ui.widget, jquery.ui.position, jquery.ui.autocomplete]
 *
 * EPFL usage: [tabs] => people pages; [overlay] => modal windows; [expose] => effect while opening header navigation.
 * 
 ...
 * 
 * File generated: Wed Oct 06 10:07:49 GMT 2010
 */
...
*
* EPFL usage: main navigation dropdown menus
*/
 ...
 *
 * EPFL usage: secondary navigation (rightnav)
 */
...
* 
* EPFL usage: URL extraction for local search when using search.epfl.ch
*/
...
*/
(function($){$.jGoogleAnalytics=function(trackerCode,options){settings=$.extend({anchorClick:false,clickEvents:null,crossDomainSelector:false,domainName:false,evalClickEvents:null,evalSubmitEvents:null,extensions:["pdf","doc","xls","csv","jpg","gif","mp3","swf","txt","ppt","zip","gz","dmg","xml"],external:"/external/",mailto:"/mailto/",download:"/download/",organicSearch:null,pageViewsEnabled:true,sampleRate:null,submitEvents:null});if(options){$.extend(settings,options)}init();function setupTracking(){var pageTracker=_gat._getTracker(trackerCode);if(settings.topLevelDomain){pageTracker._setDomainName(settings.topLevelDomain)}if(settings.sampleRate){pageTracker._setSampleRate(settings.sampleRate)}if(settings.crossDomainSelector){pageTracker._setDomainName("none");pageTracker._setAllowLinker(true);$("form"+settings.crossDomainSelector).submit(function(){pageTracker._linkByPost(this)});$("a"+settings.crossDomainSelector).click(function(){pageTracker._link($(this).attr("href"))})}if(settings.organicSearch){$.each(settings.organicSearch,function(key,val){pageTracker._addOrganic(key,val)})}pageTracker._initData();addTracking(pageTracker)}function addTracking(pageTracker){if(settings.anchorClick){$("a").each(function(){var u=$(this).attr("href");if(typeof(u)!="undefined"){var newLink=decorateLink(u);if(newLink.length){$(this).click(function(){pageTracker._trackPageview(newLink)})}}})}if(settings.clickEvents){$.each(settings.clickEvents,function(key,val){$(key).click(function(){pageTracker._trackPageview(val)})})}if(settings.evalClickEvents){$.each(settings.evalClickEvents,function(key,val){$(key).click(function(){evalVal=eval(val);if(evalVal!=""){pageTracker._trackPageview(evalVal)}})})}if(settings.evalSubmitEvents){$.each(settings.evalSubmitEvents,function(key,val){$(key).submit(function(){evalVal=eval(val);if(evalVal!=""){pageTracker._trackPageview(evalVal)}})})}if(settings.submitEvents){$.each(settings.submitEvents,function(key,val){$(key).submit(function(){pageTracker._trackPageview(val)})})}if(settings.pageViewsEnabled){pageTracker._trackPageview()}else{}}function decorateLink(u){var trackingURL="";if(u.indexOf("://")==-1&&u.indexOf("mailto:")!=0){var ext=u.split(".")[u.split(".").length-1];var exts=settings.extensions;for(i=0;i<exts.length;i++){if(ext==exts[i]){trackingURL=settings.download+u;break}}}else{if(u.indexOf("mailto:")==0){trackingURL=settings.mailto+u.substring(7)}else{var regex=/([^:\/]+)*(?::\/\/)*([^:\/]+)(:[0-9]+)*\/?/i;var linkparts=regex.exec(u);var urlparts=regex.exec(location.href);if(linkparts[2]!=urlparts[2]){trackingURL=settings.external+u}}}return trackingURL}function init(){try{var gaUrl=(location.href.indexOf("https")==0?"https://ssl":"http://www");gaUrl+=".google-analytics.com/ga.js";$.getScript(gaUrl,function(){setupTracking()})}catch(err){}}}})(jQuery);
/*!
 * jQuery Form Plugin
